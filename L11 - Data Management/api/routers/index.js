...
rootRouterV1.get("/posts", async (req, res) => {
  // crr page
...
// Lay post theo truong tim kiem
rootRouterV1.get("/posts/search-posts", async (req, res) => {
  try {
    const {
      content,
      startDate,
    ...
    if (authorId) {
      query.authorId = { $regex: authorId, $options: "i" };
    }
    ...
    const totalItems = await PostsModel.countDocuments(query);
